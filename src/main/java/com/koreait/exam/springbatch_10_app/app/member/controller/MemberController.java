...
public class MemberController {
    ...
    @PreAuthorize("isAnonymous()")  // 인증되지 않은 놈만 실행 가능 // 다른말로 하면? 로그인 안한 놈만 접근 가능.
    @GetMapping("/login")
    ...
    @PreAuthorize("isAnonymous()")  // 로그인 하지 않은 놈만 접근 가능 // 로그인 x
    @GetMapping("/join")
    ...
    @PreAuthorize("isAnonymous()")  // 로그인 하지 않은 놈만 접근 가능 // 로그인 x
    @PostMapping("/join")
    public String join(@Valid JoinForm joinForm) {  // Valid: 검증, 유효성 검사
