package com.koreait.exam.springbatch_10_app.app.handler;
...
public class CustomAuthSuccessHandler extends SimpleUrlAuthenticationSuccessHandler { // 인증 성공시의 동작을 컨트롤
    ...
    // 사용자가 인증 되지 않은 상태에서 접근하려고 했던 요청을 기억함 => 인증 성공하면 기억하는 곳으로 리다이렉트
    private final RedirectStrategy redirectStrategy = new DefaultRedirectStrategy();
    // 인증 성공 -> 다른 URL로 보냄

    @Override
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
                                        Authentication authentication) throws IOException { // 인증 성공 시 호출되는 메소드
        clearSession(request);      // 로그인 실패 시에 남아있는 에러 정보를 세션에서 제거
         ...
         */
        if (savedRequest != null) { // 사용자가 보호된 페이지에 접근 시에 사용자가 보냈던 요청이 savedRequest에 저장됨
            url = savedRequest.getRedirectUrl();
        } else if (prevPage != null && prevPage.length() > 0) {
            // 회원가입 후 바로 로그인 하면 /(home)으로 리다이렉트
        ...
        url = Ut.url.modifyQueryParam(url, "msg", memberContext.getName() + "님 환영합니다."); // 환영 메시지 추가
        redirectStrategy.sendRedirect(request, response, url);
    }

    // 로그인 실패 후 성공 시 남아있는 에러 세션 제거
    // 실패 상태가 성공 이후에 영향을 주지 않도록
